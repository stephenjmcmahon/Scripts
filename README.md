...
* **Stack Reclamation:** Assesses Cisco Catalyst stacks to identifies last stack member and checks if it can be safely removed and redeployed. SSHes into each switch, checks port utilization, PoE budget, and VLAN complexity on the candidate and receiving switch, and scores feasibility as Green/Yellow/Red. Outputs a full CSV report per run for documentation and review. Credentials are never stored and are passed at runtime via secure prompt.
